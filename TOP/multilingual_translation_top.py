...
@register_task("multilingual_translation_top")
class MultilingualTranslationTOPTask(LegacyFairseqTask):
    """
    ...
    def __init__(self, args, langs, dicts, training):
        ...
        dict_len = len(self.source_dictionary)
        self.mask = {}
        for k in ["encoder.embed_tokens.weight", "decoder.embed_tokens.weight", "decoder.output_projection.weight"]:
            self.mask[k] = torch.cat([torch.ones([64010, 1024]), torch.zeros([dict_len - 64010, 1024])], dim=0)
        ...
        if self.mask is not None:
            for n, p in model.named_parameters():
                if p._grad is None:
                    #print(n)
                    continue
                n = n.replace("module.module.", "")
                if 'embed' in n or 'decoder.output_projection' in n:
                    p._grad.masked_fill_(self.mask[n].squeeze().to(torch.bool).cuda(), 0.)
                elif 'norm' in n:
                    p._grad.zero_()
                else:
                    pass
